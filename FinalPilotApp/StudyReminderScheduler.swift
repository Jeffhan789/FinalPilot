...
// MARK: [原理] 调度器采用单例模式，集中管理所有通知的 CRUD 操作
// [原理] UserNotifications 框架是系统级服务，UNUserNotificationCenter 是单例（`UNUserNotificationCenter.current()`）。
//        本调度器作为 App 层的通知管理门面（Facade），封装了：权限检查、内容生成、时机计算、通知调度四大职责。
//        所有通知操作都经过这里，避免分散在各业务逻辑中，便于统一管理通知 ID 命名规范、防止重复调度、控制频率配额。
// [面试] "App 的通知系统怎么设计？如何保证不重复发送？"
//        答：设计要点：1) 统一调度器（本例的 `StudyReminderScheduler`），所有通知由它生成；2) 每次重新调度前先 `cancelAllPendingNotifications()`
//        清理旧通知，再用确定的 ID 生成新通知，ID 中包含业务标识（如 `daily_reminder_9`、`countdown_c310_7`），相同 ID 的通知会被覆盖；
//        3) 通知内容从 Store 实时读取，保证与 App 数据一致；4) 用 `UserDefaults` 持久化用户偏好（如提醒时段），启动时读取并重新调度。
final class StudyReminderScheduler: ObservableObject {
    ...
    // MARK: [原理] 三时段设计基于人体认知节律：早晨记忆编码、下午巩固、晚上检索
    // [原理] 认知科学研究表明，一天中的记忆效率并非均匀分布。早晨（9:00 左右）大脑经过休息，
    ...
    // [面试] "通知时间怎么选？为什么不是每小时发一次？"
    //        答：通知时间选错比不通知更糟糕。如果通知在用户忙碌时到达（如通勤、开会），会被忽略甚至引发反感，
    //        长期会导致用户关闭通知权限。选 9/14/20 的依据：1) 认知节律匹配；2) 避开工作和生活高峰；3) 数量控制——
    //        iOS 用户对通知轰炸极度敏感，一天 3 条是心理可接受的阈值。每小时一条会触发用户关闭权限，App 的通知渠道就彻底废了。
    //        如果面试官追问个性化：可以基于用户实际点击时间做强化学习（如用户总是在 10:00 点击早晨通知，
    //        可以动态调整到 10:00），但实现复杂度较高，需要后端支持。
    ...
    // MARK: [原理] 全量重新调度策略：先取消再创建，保证状态一致性
    // [原理] 本调度器采用"全量重建"策略：每次数据变更时，先取消所有 pending 通知，再重新计算并创建新通知。
    ...
    // [面试] "通知重新调度时，为什么先 cancel 再 create？有没有更好的方案？"
    //        答：先取消再创建是最简单的正确方案。因为 Store 数据变化后，通知内容（如考试倒计时天数、任务数量）可能已经变了，
    //        旧通知的信息是 stale 的。另一种方案是"增量更新"：只更新变更的部分，但实现复杂——需要维护每个通知对应的 Store 对象引用，
    //        并对比前后状态。对于本 App 这种通知量很小的场景（<20条），全量重建更简单可靠。如果面试官追问性能：
    //        iOS 的 `UNUserNotificationCenter` 的 `removeAllPendingNotificationRequests()` 是 O(1) 操作（系统只是清空数组），
    //        重建 10-20 条通知也是毫秒级，对 UI 无影响。但如果通知量达到上百条，就需要考虑增量方案了。
    ...
    // MARK: [原理] 内容差异化策略：三时段推送不同侧重点，匹配用户认知状态与执行意图
    // [原理] 早晨（<12:00）用户刚进入学习状态，前额叶皮层活跃度最高，适合推送"倒计时+任务数"形成紧迫感，
    ...
    // [面试] "通知内容怎么设计才能提高点击率？A/B 测试应该测什么？"
    //        答：点击率的底层是"相关性×紧迫感×行动清晰度"。具体设计原则：1) 个性化变量——用用户真实数据
    //        （如具体错题数、具体任务名、具体掌握度百分比），而不是通用文案， personalized 通知的点击率通常是通用的 2-3 倍；
    //        2) 场景时效——结合当前时间场景（早晨说倒计时、下午说薄弱点、晚上说错题），让用户感觉"这条通知是为现在准备的"；
    //        3) 单一行动指令——每条通知只隐含一个具体行动（"抽10分钟回顾"），降低决策成本。
    //        A/B 测试建议：先测"内容类型"（倒计时 vs 薄弱点 vs 通用鼓励），再测"文案框架"（损失框架 vs 收益框架），
    //        最后测"发送时间"（固定时段 vs 用户自定义）。不要同时测多个变量，否则无法归因。
    ...
    // MARK: [原理] 1/3/7 天倒计时策略：基于记忆唤醒曲线和备考心理学
    // [原理] 考试倒计时的通知不是简单的"还有 X 天"，而是经过心理学设计的"压力梯度"：
    ...
    // [面试] "倒计时通知为什么选 1、3、7 天？为什么不是每天发？"
    //        答：三个原因。1) 心理学上，7 天是短期记忆的巩固周期，1 天是临场记忆的峰值期，3 天是中间检查点；
    //        2) 通知配额限制：iOS 对推送通知有频率管控，每天发送会被系统限制；3) 用户体验：每天一条倒计时会被用户迅速无视
    //        （"通知疲劳"），间隔发送反而每条都有新鲜感。如果面试官追问改进：可以加入用户自定义（让用户选 1/2/3/5/7 天），
    //        或者根据科目难度动态调整（难点科目增加提醒频次）。
    ...
    // MARK: [原理] 20:30 统计推送：利用"完成感"与"进度可视化"强化内在学习动机
    // [原理] 选择 20:30 而非 23:00 或 18:00，是基于"学习活动边界"的假设：此时用户大概率已完成当天主要学习任务，
    ...
    // [面试] "学习类 App 怎么做用户留存？数据摘要推送真的有用吗？"
    //        答：数据摘要推送是留存工具箱里 ROI 很高的一招，核心原理是"自我监控效应"（Self-Monitoring）与"身份认同构建"。
    //        当用户连续多天看到"今日学习 45 分钟、完成 8/10 任务"，会内化为"我是一个坚持学习的人"的自我认知，
    //        这种身份认同是长期留存的最高级驱动力（比积分、排行榜更持久）。
    //        但有三条红线：1) 数据必须真实，伪造进度一旦被发现会彻底摧毁信任；2) 频率不能过高，每天一条是上限，
    //        否则变成噪音；3) 负面情绪保护——用户表现差时不应强调数据，而应给予情感支持（"今天状态不好，明天继续"）。
    //        技术上，这类摘要可以走 iOS Live Activity 或 Widget 实时展示，比通知打扰性更低、曝光更持续。
    /// 每天 20:30 推送学习统计摘要
    ...
    // MARK: [原理] 艾宾浩斯间隔重复：错后 60 分钟首次提醒，错开 30 分钟避免通知轰炸
    // [原理] 艾宾浩斯遗忘曲线指出：学习后 1 小时内遗忘速度最快（从 100% 降到约 44%）。
    ...
    // [面试] "错题回顾的间隔怎么设计的？艾宾浩斯曲线怎么落地到代码里？"
    //        答：核心思路是"在遗忘临界点复习"。本 App 的简化版艾宾浩斯实现：第一次回顾 60 分钟后（快速遗忘期），
    //        第二次 24 小时后（scheduleFollowUpIfNeeded 创建的 Must 任务），第三次由用户手动安排。
    //        更完整的实现可以参考 SM-2 算法：根据难度评级动态调整间隔（Easy=4天, Good=1天, Hard=1天, Again=1分钟），
    //        本 App 因为周期短（期末冲刺 1-2 周），简化为固定间隔。代码里的 `delayMinutes = 60 + (index * 30)` 就是错开调度，
    //        避免 UNUserNotificationCenter 的 64 条通知上限被快速耗尽。
    ...
    // MARK: [原理] 即时正反馈：行为发生后秒级响应，强化操作性条件反射回路
    // [原理] 即时通知与定时通知的本质区别：定时通知是"外部时钟驱动"（系统决定何时推），
    ...
    // [面试] "即时通知和定时通知分别在什么场景用？设计原则有什么不同？"
    //        答：即时通知用于"行为强化"——在用户完成关键动作后立刻给予正向反馈（完成任务、达成连胜、获得成就），
    //        增强自我效能感（Self-Efficacy）；定时通知用于"习惯养成"——在用户可能遗忘的时间点提醒执行目标行为（早晨学习、晚上总结）。
    //        设计原则差异：即时通知要"惊喜感"（文案多变、带 Emoji、语气活泼、不固定时间），
    //        定时通知要"稳定感"（固定时间、固定结构、可预期）。混用会导致用户体验认知混乱。
    //        技术注意：即时通知走 `UNNotificationRequest` 的即时触发（`trigger: nil` 或极短时间），
    //        但 iOS 对高频即时通知有隐性限制，建议本地做去抖（如 30 秒内最多发 1 条），避免被系统限流。
    ...
    // MARK: [原理] 用户偏好持久化通过 UserDefaults 实现，但这里用逗号分隔字符串而不是数组
    // [原理] `UserDefaults` 原生支持 `String` 和 `Array` 存储，但本例选择将 `[Int]` 编码为逗号分隔的字符串（如 "9,14,20"）。
    ...
    // [面试] "用户偏好存储用什么方案？Core Data 还是 UserDefaults？"
    //        答：选择依据是数据类型。UserDefaults 适合轻量配置（bool、int、string、小数组），特点是读写快、自动同步、无需模型；
    //        Core Data 适合结构化数据、需要查询/关联/迁移的场景。本例的提醒时段是简单的整数列表，用 UserDefaults 最合适。
    //        但注意 UserDefaults 的容量限制：虽然 Apple 没有公开上限，但存储大量数据（如图片、长文本）会拖慢启动速度。
    //        最佳实践：只存配置和状态，不存业务数据。本 App 中，学习记录、考试数据存在 Core Data，通知时段等配置存在 UserDefaults。
    /// 更新用户自定义提醒时段并重新调度
